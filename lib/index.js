...
Simulcast.prototype._parseSimLayers = function (mLine) {
    let simGroup = mLine.ssrcGroups &&
        mLine.ssrcGroups.find(group => group.semantics === "SIM");
    if (simGroup) {
...
Simulcast.prototype._buildNewToOldSsrcMap = function (newSsrcList, oldSsrcList) {
    let ssrcMap = {};
    for (let i = 0; i < newSsrcList.length; ++i) {
        let newSsrc = newSsrcList[i];
        let oldSsrc = oldSsrcList[i] || null;
...
Simulcast.prototype._fillInSourceDataFromCache = function(mLine) {
    let newSimSsrcs = this._parseSimLayers(mLine);
    let newMsid = getSsrcAttribute(mLine, newSimSsrcs[0], "msid");
    let newCname = getSsrcAttribute(mLine, newSimSsrcs[0], "cname");
    let ssrcsToReplace = this._buildNewToOldSsrcMap(newSimSsrcs, this.ssrcCache);
    // New sdp might only have 1 layer, so not every cached ssrc will have a new one
    //  to replace directly
    let ssrcsToAdd = this.ssrcCache
        .filter((ssrc) => Object.values(ssrcsToReplace).indexOf(ssrc) === -1);
...
Simulcast.prototype._generateSourceData = function(mLine, primarySsrc, doingRtx) {
    ...
    // Generate sim layers
    let simSsrcs = [];
    for (let i = 0; i < this.options.numOfLayers - 1; ++i) {
...
Simulcast.prototype._restoreSimulcast = function(mLine) {
    ...
    var primarySsrc;
	let numSsrcs = mLine.ssrcs
		.map(ssrcInfo => ssrcInfo.id).filter(
    ...
    let seenPrimarySsrc = this.ssrcCache.indexOf(primarySsrc) !== -1;
